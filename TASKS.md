...
13. **[2026-08-24] ⚠️ ÖNEMLİ — Karakter oluşturmada stat doğrulaması yok, client istediği değeri gönderebiliyor (hile açığı).** `backend/routes/character.js:30-33` — `isValidAttributeSet()` sadece her attribute'un `Number.isFinite` olduğunu kontrol ediyor, ÜST/ALT SINIR YOK. `POST /api/character/create`'e `attributes: {str:99, dex:99, ...}` göndererek canlı production'da (`dnd-game-backend-sz9e.onrender.com`) doğrudan test ettim — istek 200 ile kabul edildi, karakter tam olarak 99/99/99/99/99/99 stat'larla oluşturuldu (test karakterini sonra reset'ledim, prod'da kalıntı yok). Meşru bir zar atışı D20 (1-20) + ırk bonusu (en fazla +2) ile ~1-22 aralığında olmalı, ama backend client'ın gönderdiği HERHANGİ bir sayıyı güveniyor. Bu, oyunun kendi zar sistemini (Faz 3-A'nın D20 zar atma özelliği) tamamen anlamsız kılan bir hile açığı — herhangi biri devtools'tan istek gönderip god-mode karakter yaratabilir. Öneri: `isValidAttributeSet`'e makul bir aralık kontrolü ekle (örn. her attribute 1-22 arası, ya da daha güvenlisi: sunucu ATTRIBUTES'i asla client'tan güvenip almasın, her zaman kendi tarafında yeniden hesaplasın/doğrulasın). Küçük ama ÖNEMLİ bir iş — oyun bütünlüğü açığı, öncelik verilmeli.
