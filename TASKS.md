...
- [ ] **İkon tabanlı envanter/ekipman** (ASSET HAZIR — commit 606145f): İkonlar `assets/tgstation-icons/` altında (tgstation/SS13 kaynağı, CC BY-SA 3.0 — atıf ve yükümlülükler `assets/tgstation-icons/CREDITS.md`'de, oyunun bir yerine "Hakkında/Kredi" notu eklenmeli). İki alt klasör: `obj-clothing/` (envanterdeki eşya ikonları, kullanılacak asıl set) ve `mob-clothing/` (karakter üstünde giyili görünüm, muhtemelen bu fazda gerekmez). Dosyalar `.dmi` uzantılı ama içerik olarak standart PNG (çoklu-durum sprite sheet + metinsel metadata) — `file <dosya>.dmi` ile PNG olduğunu doğrulayabilirsin, direkt `<img>`/CSS `background-image` ile kullanılabilir, sadece sprite sheet'ten doğru state/frame'i kesmek gerekir (DMI metadata'sını okumak için bir PNG text-chunk parser ya da BYOND DMI format dokümantasyonuna bakılabilir — basit bir yaklaşım: her state genelde sabit boyutlu (örn. 32x32) kareler halinde diziliyor, ilk state'i almak çoğu durumda yeterli).
  Slot düzeni: `obj-clothing/` altındaki klasör adları zaten SS13 slotlarına karşılık geliyor — head/, suits/ (zırh), gloves.dmi, shoes.dmi, glasses.dmi, masks.dmi, ears.dmi, neck.dmi, belts.dmi, back (mob-clothing'de var, obj tarafında da benzer), accessories.dmi, under/ (üst giysi). Bunları mevcut basit 6 slotlu sistemden (baş/göğüs/kol/bacak/ayak/el) SS13 tarzı genişletilmiş slot yapısına geçirirken referans olarak kullan.
  Grid tabanlı, ikonlu, "SWAP" (yer değiştirme) özellikli envanter arayüzü hedefleniyor. Artık BAŞLAYABİLİRSİN.
