...
15. **[2026-08-24] İksir/tüketilebilir eşyaların envanterde hiç ikonu/emoji'si yok — sadece metin görünüyor.** `backend/data/itemSlots.js:31` — `"İksir (Küçük İyileştirme)": null` (slot yok, dolayısıyla `getIconForSlot(null)` da `null` dönüyor). `frontend/src/components/CharacterCard.tsx:298` — `{item.icon && <img .../>}` koşullu render: `icon` `null`/falsy ise HİÇBİR ŞEY render edilmiyor (fallback emoji YOK). Faz 5'te "el" slotundaki silahlara (asset setinde ikon yoktu) ⚔ emoji fallback'i eklenmişti, kullanıcı Faz 8'de "birçok şey için emoji kullanmanı istiyorum" demişti (o zaman envanter aksiyonlarına/başlıklara emoji eklendi) — ama tüketilebilir eşyalar (iksir gibi, slotsuz kategori) bu kapsamın dışında kalmış, hâlâ görsel olarak "çıplak". Öneri: `CharacterCard.tsx`'te `item.icon` yoksa (ve slot da yoksa, yani tüketilebilir kategoride) basit bir 🧪 emoji fallback'i göster (silahlardaki ⚔ deseniyle simetrik). Küçük/net bir iş.
