...
17. **[2026-08-24] "Nasıl Oynanır?" yardım modalında hiç klavye desteği yok — Esc ile kapanmıyor, focus yönetimi yok.** `frontend/src/components/HelpModal.tsx` ve `App.tsx`'te `Escape`/`onKeyDown`/`tabIndex`/`focus` için hiçbir sonuç bulamadım (grep ile doğruladım). Modal açıldığında sadece fare ile (❓ butonu, overlay tıklama, kapat butonu) kapatılabiliyor — klavye kullanan biri (ya da Tab ile gezinen biri) modalı Esc ile kapatamıyor, focus modal içine "hapsedilmiyor" (arkadaki sayfaya Tab ile kaçılabilir). #10'daki (aria-label) erişilebilirlik bulgusunun doğal bir devamı — bu sefer ekran okuyucu değil, klavye kullanılabilirliği. Öneri: modal açıkken `document`'a bir `keydown` dinleyicisi ekleyip `Escape` tuşunda kapat; ekstra puan için ilk odaklanabilir elemana (örn. kapat butonu) otomatik focus. Küçük/net bir iş.
