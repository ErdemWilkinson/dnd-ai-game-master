...
9. **[2026-08-23] Game Over ekranı bomboş — hiçbir macera özeti/kapanış yok.** `frontend/src/components/GameOverScreen.tsx` sadece "{isim}, seviye {level}'de düştü" gösterip "Yeniden Başla" butonuna geçiyor — kaç düşman yenildiği, kaç karşılaşma temizlendiği, ne kadar XP kazanıldığı gibi hiçbir özet yok. Backend zaten bu verileri tutuyor (`character.xp`, `scene.encounterIndex`) ama Game Over anında kullanıcıya gösterilmiyor. Onboarding eksikliği (#5, zaten düzeltildi) oyunun BAŞLANGICI içindi, bu SONU ile ilgili — bir maceranın "boş bir şekilde bitmesi" hissi kalıcı kullanıcı bağlılığı için kayıp bir fırsat. Öneri: Game Over ekranına 1-2 satırlık bir özet ekle (örn. "3 karşılaşma temizledin, 85 XP kazandın, seviye 2'ye ulaştın"). Küçük/net bir iş.
