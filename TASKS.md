...
16. **[2026-08-24] `/item/throw` koordinatları grid sınırlarına/menzile göre hiç doğrulamıyor.** `backend/routes/scene.js` — `POST /item/throw` sadece `typeof x/y === "number"` kontrolü yapıyor, `GRID_WIDTH`/`GRID_HEIGHT` sınırı YOK, oyuncu pozisyonuna göre bir menzil kontrolü de YOK. Karşılaştırma: `/attack` bitişiklik kontrolü yapıyor, `/scene/cast` (Ateş Topu) `range:3` kontrolü yapıyor — ama `/item/throw` hiçbirini yapmıyor. Bir client `x:99999, y:-500` gibi grid dışı bir koordinat gönderirse, `scene.loot`'a görünmez/erişilemez bir loot eşyası ekleniyor — eşya kalıcı olarak "kayboluyor" (kullanıcı deneyimi hatası, güvenlik açığı değil ama gerçek bir fonksiyonel bug). Öneri: `x`/`y`'nin `0 ≤ x < GRID_WIDTH` ve `0 ≤ y < GRID_HEIGHT` aralığında olduğunu doğrula, isteğe bağlı olarak makul bir fırlatma menzili de eklenebilir (örn. Ateş Topu'nun range:3'üyle tutarlı). Küçük/net bir iş.
