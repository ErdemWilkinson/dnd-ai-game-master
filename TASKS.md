...
11. **[2026-08-24] Karakter ismi ve chat mesajlarında hiç uzunluk sınırı yok.** `backend/routes/character.js:54` ve `backend/routes/chat.js:34` sadece boş/whitespace kontrolü yapıyor (`!name.trim()`), üst sınır YOK. Faz 9'daki genel rate-limit istek SIKLIĞINI sınırlıyordu, bu farklı bir sorun: tek bir istekte devasa bir string (örn. birkaç MB'lık karakter ismi ya da chat mesajı) gönderilebilir — bu hem DB'yi şişirir hem (chat mesajı için) doğrudan Gemini prompt'una gidip pahalı/başarısız bir AI çağrısına yol açabilir, hem de frontend'de dev bir metin bloğu render sorunlarına neden olabilir. Öneri: `name` için makul bir üst sınır (örn. 50 karakter), `message` için (örn. 500 karakter) — aşılırsa 400 dönsün. Küçük/net bir iş.
