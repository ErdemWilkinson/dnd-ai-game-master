...
12. **[2026-08-24] Hırsız (Rogue) sınıfı hiçbir zaman büyü kullanamıyor ama arayüzde büyü paneli görünüyor.** `backend/data/dnd.js` — Rogue `baseMana: 3`. `backend/data/spells.js` — her iki büyünün de (`fireball`, `heal`) `manaCost: 4`. Frontend'de büyü paneli `mana.max > 0` koşuluyla gösteriliyor (Faz 6-C notlarına göre) — Rogue'un `mana.max` değeri 3 (>0) olduğu için panel görünüyor ama 3 mana ile hiçbir zaman 4 mana harcayan bir büyü seçilemiyor (buton hep "yetersiz mana" ile disabled kalıyor, kalıcı olarak). Yani Hırsız oyuncusu hiç kullanamayacağı, sürekli devre dışı görünen bir büyü paneliyle karşı karşıya — kafa karıştırıcı/gereksiz. Öneri: görünürlük koşulunu `mana.max > 0` yerine `mana.max >= en ucuz büyünün maliyeti` yap (ya da basitçe Rogue'un `baseMana`'sını 4'e çıkar, ama bu "hırsızın çok az büyü hissi olsun" tasarım niyetini bozabilir — koşulu düzeltmek daha güvenli). Küçük/net bir iş.
