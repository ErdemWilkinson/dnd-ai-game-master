...
10. **[2026-08-24] Erişilebilirlik (accessibility) sıfıra yakın — `aria-label` neredeyse hiç yok.** `frontend/src/` genelinde grep ile sadece 1 `aria-label` bulundu (tüm bileşenlerde 0). Faz 8'de kullanıcının isteğiyle arayüz emoji-ağırlıklı hale getirildi (HP/Mana ikonları, slot etiketleri, aksiyon butonları) — bu görsel olarak iyi ama ekran okuyucu kullanan biri için emoji-only butonlar/ikonlar anlamsız kalıyor (örn. sadece "⚔" gösteren bir buton, "Saldır" diye okunmuyor). Öneri: en azından ikon-only/emoji-only interaktif elemanlara (paper-doll slotları, aksiyon butonları, header'daki ❓ yardım butonu) `aria-label` eklensin — geniş bir a11y denetimi şart değil, en kritik etkileşimli elemanlar yeterli. Küçük/mekanik bir iş, coder alabilir.
