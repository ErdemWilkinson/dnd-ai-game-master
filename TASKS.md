...
14. **[2026-08-24] Son karşılaşmayı (Ejderha İni) temizleyince hiçbir "kampanyayı tamamladın" anı yok — sessizce ilk karşılaşmaya (Terk Edilmiş Mahzen) geri sarılıyor.** `backend/data/sceneFactory.js:41` — `createScene()` `ENCOUNTERS[encounterIndex % ENCOUNTERS.length]` ile modulo alıyor, `checkEncounterCleared()` (`routes/scene.js`) her geçişte aynı genel "Karşılaşma temizlendi! Yeni bir alana geçiliyor: X." mesajını basıyor — 4. (son/en zor, Ejderha) karşılaşma temizlenip 1.'ye dönüldüğünde de bu FARK EDİLMİYOR, oyuncu en zor içeriği bitirmiş olmasına rağmen hiçbir özel tebrik/kapanış anı yaşamıyor, sanki hiçbir şey olmamış gibi devam ediyor. (Game Over ekranındaki macera özeti — #9, zaten düzeltildi — sadece ÖLÜNCE gösteriliyor, bu farklı: bir TAM TUR'u BAŞARIYLA tamamlamakla ilgili.) Öneri: `checkEncounterCleared`'da `(scene.encounterIndex + 1) % ENCOUNTERS.length === 0` olduğunda (yani tam bir tur tamamlandığında) farklı/özel bir tebrik cümlesi döndür (örn. "Tüm bölgeyi temizledin! Kahramanlığın efsaneleşiyor... ama tehlike hiç bitmiyor, yeni bir tehdit beliriyor: X."). Küçük/net bir iş.
